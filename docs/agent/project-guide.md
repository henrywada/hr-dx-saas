# HR-DX SaaS プロジェクトガイド

> この文書は `CLAUDE.md` から移した詳細ガイドです。エージェントは必要な節だけ読んでください。
> 常時読む索引はリポジトリ直下の `CLAUDE.md` です。
...
| `DROP TABLE` / `DROP SCHEMA` / `TRUNCATE` の実行                | 既存データが復旧不能な形で消滅する             |
| `WHERE` 句のない `DELETE` / `UPDATE`                            | 全行破壊。範囲指定は必ず明示する               |
| 既存テーブルに対する `CREATE TABLE`（`IF NOT EXISTS` なし）     | 定義の上書き・データ消失につながる             |
...
**ローカル・本番いずれの Supabase にも、ユーザーが手作業で投入した実データが入っている。データ消失は本プロジェクトで最も重大な事故であり、実際に発生している（テーブル全消失 → バックアップからの復旧も不完全に終わった）。**

ユーザーが毎回「既存データは絶対に消さないでください」と書かなくても、以下は**常に有効な既定ルール**として扱う。

### DDL / DML の実行前に必ず守ること

1. **対象 DB を宣言してから実行する** — 「これはローカル（`127.0.0.1:55422`）に対する操作です」「これは本番 Supabase に対する操作です」と明示し、破壊的操作なら**実行前にユーザーの承認を取る**。どちらの DB か判断がつかない場合は実行せずに確認する。
2. **テーブル作成は `CREATE TABLE IF NOT EXISTS` を使う** — 既存テーブルがある場合は一切変更を加えない。カラム追加は `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` で行う。
3. **削除系は使わない** — `DROP` / `TRUNCATE` / 範囲無指定の `DELETE` は、ユーザーが対象を具体的に指定して明示的に依頼した場合のみ。その際も**影響件数を `SELECT COUNT(*)` で先に提示**し、承認を得てから実行する。
4. **「作り直す」提案をしない** — スキーマ不整合の解決策として reset / 再作成を提案しない。マイグレーションの積み上げで解決する。
5. **調査は読み取り専用から始める** — 原因調査の段階では `SELECT` のみを実行する。

### 事故が起きた場合

推測でリカバリ操作を重ねない。**まず現状（どのテーブルの何件が失われたか）を `SELECT COUNT(*)` で確定させ、バックアップの有無を確認し、復旧計画をユーザーに提示してから実行する。** バックアップの所在は `backups/backup_YYYYMMDD_HHMMSS/`（日付別スナップショット）と `supabase/backups/`（`full_backup_*.sql` 等のダンプ）。
