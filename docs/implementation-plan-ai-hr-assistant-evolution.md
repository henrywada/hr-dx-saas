...
一般従業員の TOP 画面にある**「お問合せ」（inquiry-chat）は本機能とは別物**である。

| 項目         | お問合せ（inquiry-chat）                            | AI人事アシスタント（hr-assistant）                               |
| ------------ | --------------------------------------------------- | ---------------------------------------------------------------- |
| 対象ユーザー | 一般従業員                                          | テナント管理者（人事）                                           |
| RAG 参照先   | `tenant_rag_documents/chunks`（社内規定等）**のみ** | テナント資料 + 法令ナレッジ（本設計で追加）                      |
| テナント分離 | 完全分離                                            | チャット履歴・テンプレは完全分離。法令ナレッジのみ全テナント共有 |

本設計の変更（法令ナレッジのマージ・質問テンプレート）は **hr-assistant 側のみ**に
適用し、**inquiry-chat には一切手を入れない**。embedding ユーティリティと定数
（`inquiry-chat/embedding.ts`, `constants.ts`）を import して再利用するのは
従来どおりだが、inquiry-chat の検索経路・プロンプトは変更しない。
...
7. **SerpAPI 残量チェック**: ラン冒頭で SerpAPI Account API
   （`https://serpapi.com/account.json`）を呼び、`plan_searches_left` が
   閾値（50 回）未満なら、既存の `send-email` Edge Function 経由で
   警告メールを送信する（宛先は環境変数 `SAAS_ALERT_EMAIL`、
   本番設定値: `wada007@gmail.com`）。残量不足でもランは継続し、
   検索失敗はソース単位でスキップする
...
## 13. 決定事項（2026-07-09 確認済み）

1. **GEMINI_API_KEY の本番設定** — 実施する（Gemini 移行の残タスクとして先行対応）。
   本機能の前提条件
2. **法令ナレッジの全テナント共有** — 承認。公開情報のみのため共有とする。
   業種別の絞り込みニーズが出たら `hr_law_sources` にカテゴリ列を追加して対応
3. **SerpAPI クォータ** — 必要に応じてプランをアップグレードする。
   残量が閾値（50 回）を下回ったら `wada007@gmail.com` へ警告メールを送信する
   （7.1 の残量チェック参照。宛先は `SAAS_ALERT_EMAIL` 環境変数で管理）
4. **inquiry-chat（お問合せ）とは別機能** — 一般従業員 TOP の「お問合せ」は
   従業員向け・テナント完全分離の RAG であり、本設計の対象外。変更を加えない
   （3 章「関連機能との区別」参照）

## 14. オープンクエスチョン（残）

1. Edge Function 内で Gemini SDK を使うか REST 直叩きか
   （Deno 環境のため `@google/genai` の Deno 互換性を実装時に確認。
   非互換なら fetch で REST API を叩く）
2. ダッシュボードカード（Phase 3）の配置先 — /adm トップの既存カードグリッドの
   どの位置に入れるか（UX 判断）
