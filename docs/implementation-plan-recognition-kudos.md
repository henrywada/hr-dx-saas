...
4. 通知（受信者への社内お知らせ連携 or 簡易通知） — ✅ 2026-06-30 完了（`createKudos` → 個人宛 `announcements` 発行、`KudosPendingNotice` と併用）

**Should**
5. 部署別・個人別の称賛件数ランキング — hr-kpi エンゲージメント指標追加 ✅ 2026-06-30
6. バリュータグのテナント管理者によるカスタマイズ

**Could（後続フェーズ）**
7. 月間MVP自動集計（社内イベント・表彰機能と連携） — ✅ 2026-06-30 完了（`/adm/kudos-stats` と `/adm/events-awards` で候補表示）
