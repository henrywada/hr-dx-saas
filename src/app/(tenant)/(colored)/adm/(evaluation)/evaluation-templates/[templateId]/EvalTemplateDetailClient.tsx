...
export function EvalTemplateDetailClient({ template }: Props) {
  ...
  const groupedItems = AXES.reduce<Record<EvaluationAxis, EvaluationTemplateItem[]>>(
    (acc, axis) => {
  ...
  function startEdit(item: EvaluationTemplateItem) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await addCustomEvaluationItem({
  ...
  function handleUpdate(id: string) {
    ...
    setError(null)
    startTransition(async () => {
      const result = await updateEvaluationTemplateItem({
        id,
        template_id: template.id,
        name: editName,
  ...
  function handleDelete(id: string, name: string) {
    ...
    setError(null)
    startTransition(async () => {
      const result = await deleteEvaluationTemplateItem({ id, template_id: template.id })
      if (!result.success) {
        setError('error' in result ? result.error : '不明なエラー')
        return
      }
    ...
    <div className="space-y-6">
                      ...
                      <td className="px-4 py-2.5">
                        {editingId === item.id ? (
                          ...
                          <span className="text-sm font-medium text-gray-800">
                            ...
                            {item.is_custom && (
                              <span className="ml-1 inline-flex items-center rounded-full bg-orange-50 px-1.5 py-0.5 text-xs text-orange-600">
                                カスタム
                              </span>
                            )}
                          </span>
      ...
      {showAddForm ? (
        <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
          <h3 className="mb-3 text-sm font-semibold text-gray-800">カスタム評価項目を追加</h3>
