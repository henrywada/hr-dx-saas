    ...
    <div className="space-y-6 animate-in fade-in duration-500 slide-in-from-bottom-4">
      ...
      {summary.maskedDepartmentCount > 0 && (
        <div className="flex items-start gap-3 bg-amber-50/70 border border-amber-200 rounded-2xl p-4 px-5">
          ...
          <div>
            <p className="text-sm font-bold text-amber-800">プライバシー保護について（匿名化）</p>
            <p className="text-xs text-amber-700 mt-1 leading-relaxed">
              ※法令に基づき、回答者が10名未満の部署は個人の特定を防ぐため単体での結果を非表示とし、「その他（匿名）」として統合して表示しています。
            </p>
          </div>
        </div>
      )}

      {/* 部署別ストレス・ヒートマップ（メイン機能） */}
      ...
      {maskedDepts.length > 0 && (
        <div className="bg-gray-50 rounded-2xl border border-gray-200 p-5 mt-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-bold text-gray-600">合算対象・その他（10名未満部署）</p>
          </div>
...
function PageHeader() {
  return (
    <div className="relative pl-5">
      <div className="absolute left-0 top-1 bottom-1 w-1.5 bg-gradient-to-b from-blue-500 to-cyan-600 rounded-full" />
