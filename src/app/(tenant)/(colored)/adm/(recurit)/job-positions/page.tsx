      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            【無料求人サイトへ自動連携】：ステータス「公開中」の求人を、無料の求人サイトへ自動掲載します。
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
        {jobs.length === 0 ? (
          <div className="p-10 text-center text-gray-500">
            ...
            <p className="text-sm border inline-block px-4 py-2 rounded bg-gray-50">
              右上の「新規求人を作成」から下書きを作成してください。
            </p>
          </div>
        ) : (
            ...
            <tbody>
              {jobs.map(job => (
                <tr key={job.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{job.employment_type || '-'}</td>
                  ...
                  <td className="px-6 py-4">{formatDateTimeInJST(job.updated_at)}</td>
                  <td className="px-6 py-4 flex items-center gap-3">
                    <Link
                      href={`/adm/job-positions/${job.id}`}
