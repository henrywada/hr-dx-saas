...
import { runSlideVideoUpload } from '@/features/e-learning/slide-media-upload'
...
export async function POST(
  ...
  try {
    const url = await runSlideVideoUpload({ slideId, courseId, file })
    return NextResponse.json({ ok: true, url })
  } catch (err) {
    const message = err instanceof Error ? err.message : '動画のアップロードに失敗しました'
