/**
 * POST /api/overtime/applications/:id/reject — 残業申請を却下（上長・同一部署スコープ）
 */
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: row, error: fetchErr } = await supabase
    ...
    .select('id, tenant_id, status, employee_id, work_date')
    .eq('id', id)
    .maybeSingle()
  ...
  if (row.status !== '申請中') {
    return NextResponse.json({ error: '申請中の申請のみ却下できます' }, { status: 409 })
  }
  ...
  if (updErr || !updated) {
    console.error('overtime reject:', updErr)
    return NextResponse.json({ error: '却下の更新に失敗しました' }, { status: 500 })
  }

  return NextResponse.json({ id: updated.id, status: updated.status as '却下' })
}
