/**
 * POST /api/overtime/applications/:id/request_correction — 修正依頼（上長・同一部署スコープ）
 */
...
const bodySchema = z.object({
  ...
  comment: z.string().min(1, 'コメントは必須です').max(2000),
  suggested_hours: z.number().positive().max(999).optional(),
})
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const correctedHours =
    parsed.data.suggested_hours ?? Number(row.requested_hours ?? 0)
  ...
  if (updErr || !updated) {
    console.error('overtime request_correction:', updErr)
    return NextResponse.json({ error: 'ステータスの更新に失敗しました' }, { status: 500 })
  }

  const { error: corrErr } = await supabase.from('overtime_corrections').insert({
    application_id: id,
    corrected_hours: correctedHours,
    correction_reason: commentText,
    corrected_by: parsed.data.supervisor_id,
  })

  if (corrErr) {
    console.error('overtime_corrections insert:', corrErr)
    return NextResponse.json({ error: '修正履歴の登録に失敗しました' }, { status: 500 })
  }

  return NextResponse.json({ id: updated.id, status: updated.status as '修正依頼' })
}
