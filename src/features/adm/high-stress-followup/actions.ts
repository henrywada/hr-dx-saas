...
export async function createInterviewRecord(payload: CreateInterviewRecordPayload) {
  ...
  if (
    ...
    (!payload.doctorOpinion || payload.doctorOpinion.trim() === '')
  ) {
  ...
  if (payload.status === 'scheduled') {
    ...
    if (!slot) {
      throw new Error(`指定の日時（${dateStr} ${timeStr}）は産業医の稼働時間外です。`)
    }
    if (slot.isBooked) {
      throw new Error(`指定の日時枠（${slot.startTime.slice(0, 5)}〜）は既に予約が入っています。`)
    }
  }
...
export async function updateInterviewRecord(
  ...
  if (payload.interviewDuration !== undefined)
    updateData.interview_duration = payload.interviewDuration
 ...
 */
export async function fetchActuallyAvailableSlotsForDate(date: string, doctorId?: string) {
  const user = await getServerUser()
  const targetId = doctorId || user?.employee_id
  if (!targetId) return []
...
/**
 * 面談準備用の関連情報（コンディション記録・パルスサーベイ・残業時間履歴）を取得
 * company_doctor / company_nurse のみ許可（コンディション記録は本人以外に開示しない設計の例外）
 */
export async function fetchEmployeeRelatedInfo(employeeId: string): Promise<EmployeeRelatedInfo> {
  const user = await getServerUser()
  if (!user?.tenant_id || !['company_doctor', 'company_nurse'].includes(user.appRole ?? '')) {
    throw new Error('Unauthorized')
  }
