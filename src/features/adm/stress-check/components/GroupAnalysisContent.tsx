...
export default function GroupAnalysisContent({
  ...
  const highRiskGroups = groups.filter(g => g.health_risk > 120)

  // 平均健康リスク（加重平均）と前回比
  const totalMembers = groups.reduce((s, g) => s + g.member_count, 0)
  const currentAvgRisk =
    totalMembers > 0
      ? groups.reduce((s, g) => s + g.health_risk * g.member_count, 0) / totalMembers
      : 0
  const prevRows = groups.filter(g => g.previous_health_risk != null)
  const prevTotalMembers = prevRows.reduce((s, g) => s + g.member_count, 0)
  const previousAvgRisk =
    prevTotalMembers > 0
      ? prevRows.reduce((s, g) => s + (g.previous_health_risk ?? 0) * g.member_count, 0) /
        prevTotalMembers
      : null
  const riskDiff =
    previousAvgRisk != null ? currentAvgRisk - previousAvgRisk : null
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-6">
          <p className="text-sm text-gray-500">部署数</p>
        ...
        <Card className="p-6">
          <p className="text-sm text-gray-500">高リスク部署</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="p-6">
            <h2 className="text-xl font-semibold mb-4">部署別健康リスク・ヒートマップ（全国平均比）</h2>
      ...
      <Card className="p-6">
        ...
        {highRiskGroups.length > 0 ? (
          ...
          <p className="text-gray-500 py-4">高リスク部署はありません</p>
        )}
      </Card>
    </div>
  )
