      ...
      {groups.map((group) => {
        ...
        return (
          <div
            ...
            className={`p-6 rounded-2xl border-2 cursor-pointer transition-all active:scale-95 ${getColorClass()} ${isSelected ? 'ring-2 ring-blue-400 ring-offset-2' : ''}`}
          >
            ...
            <div className="mt-6 text-center">
              <div className="text-6xl font-bold tracking-tighter">{suppressed ? '—' : risk}</div>
            ...
            <div className="mt-6 text-center text-xs opacity-75">
              {suppressed
                ? '集団分析を表示する人数に達していません'
                : `高ストレス率 ${group.high_stress_rate ?? 0}%`}
            </div>
          </div>
        )
      })}
