...
type Props = {
  /** false のときはメールのみ（登録済み RAG 文書がないテナント） */
  aiChatEnabled?: boolean
}

export function HrInquiryModal({ aiChatEnabled = true }: Props) {
  const [open, setOpen] = useState(false)
  const [tab, setTab] = useState<Tab>(() => (aiChatEnabled ? 'chat' : 'mail'))

  useEffect(() => {
    if (!aiChatEnabled) setTab('mail')
  }, [aiChatEnabled])
        ...
        <DialogHeader className="shrink-0 border-b border-neutral-100 px-6 pb-4 pt-6 pr-14 sm:px-8 sm:pr-16">
          ...
          <DialogPrimitive.Description className="text-sm text-neutral-600">
            {aiChatEnabled
              ...
              : '人事担当へのメール送信ができます。（制度文書が登録されると AI チャットもご利用いただけます）'}
