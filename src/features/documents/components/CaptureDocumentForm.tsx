...
import { CARD_KEYS, type CardKey } from '@/features/documents/plugins/business_card/plugin'
...
export function CaptureDocumentForm({
  ...
  const canAnalyze = Boolean(frontImage) && flowStatus !== 'uploading' && flowStatus !== 'analyzing'
  ...
  return (
    // スマホは AppLayout の左右 24px を相殺して画面幅いっぱいにする
    <div className="capture-form-shell -mx-[24px] flex w-[calc(100%+48px)] min-w-0 flex-col gap-5 bg-white px-3 py-6 pb-12 text-gray-900 md:mx-auto md:w-full md:max-w-md md:px-6">
      <div className="flex items-center justify-between gap-3">
        <div>
      ...
      <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h2 className="text-sm font-bold text-gray-900">{roleLabel(captureRole)}を撮影</h2>
            <p className="mt-1 text-xs text-gray-500">表面は必須、裏面は任意です。</p>
          </div>
      ...
      {frontImage && (
        <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
          ...
          {captureRole === 'back' && !backImage && (
            <p className="text-xs text-gray-500">裏面が不要な場合は、このまま解析に進めます。</p>
          )}
        </section>
      )}
