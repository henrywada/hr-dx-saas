...
interface PurchaseOrderAlbumProps {
...
function emptyHeader(): Record<PurchaseOrderHeaderKey, string> {
  return Object.fromEntries(PURCHASE_ORDER_HEADER_KEYS.map((key) => [key, ""])) as Record<
    PurchaseOrderHeaderKey,
    string
  >;
}

function normalizeHeader(value: Record<string, string>): Record<PurchaseOrderHeaderKey, string> {
...
export function PurchaseOrderAlbum({
  documentType,
  userId,
  isManager,
  initialOpenId,
  ...
  const [headerDraft, setHeaderDraft] = useState<Record<PurchaseOrderHeaderKey, string>>(emptyHeader);
  ...
  const loadPage = useCallback(
    async (offset: number, append: boolean) => {
      ...
      try {
        ...
        console.error("purchase_order list load failed", err);
  ...
  const loadDetail = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        console.error("purchase_order detail load failed", err);
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      a.href = url;
      a.download = `purchase_orders_${stamp}.csv`;
    ...
    <div className="mx-auto max-w-5xl px-6 py-8 pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-ink">発注書ホルダー</h1>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <Link
            href={`${APP_ROUTES.TENANT.TOOL_DOCUMENTS_NEW}?type=purchase_order`}
      ...
      <p className="mt-2 text-sm text-ink-soft">
        読み取った発注書を一覧表示します。タップで詳細・編集・再解析・削除ができます。
      </p>
            ...
            <li key={item.id} className="relative">
              ...
              >
                <input
                  ...
                  aria-label={`${item.title || "発注番号未設定"}を選択`}
                />
              </label>
              <button
                type="button"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5 truncate text-sm font-medium text-ink">
                    <span className="truncate">{item.title || "発注番号未設定"}</span>
                  ...
                  <p className="mt-0.5 truncate text-xs text-ink-soft">
                    {item.counterparty || "発注先未設定"}　{formatDate(item.contextDate)}
                  </p>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-medium text-signal">発注書ホルダー</p>
                <h2 id="purchase-order-detail-title" className="text-base font-semibold text-ink">
                  {detail?.title || selectedSummary?.title || "発注書詳細"}
                </h2>
                <p className="mt-1 text-xs text-ink-soft">
                  {detail
                    ? `${detail.counterparty || "発注先未設定"} / ${formatDate(detail.contextDate)} / 更新 ${formatTimestamp(detail.updatedAt)}`
                    : "詳細を読み込んでいます"}
                </p>
              </div>
              <button
