...
export function PurchaseOrderCaptureForm({ tenantId, userId }: PurchaseOrderCaptureFormProps) {
  ...
  const canAnalyze = pages.length > 0 && flowStatus !== 'uploading' && flowStatus !== 'analyzing'
  ...
  async function handleSave() {
    ...
    try {
      ...
      tmpPagesRef.current = []
      setTmpPages([])
      router.push(`${APP_ROUTES.TENANT.TOOL_DOCUMENTS}?type=purchase_order`)
    } catch (err) {
      ...
      <div className="capture-form-shell -mx-[24px] flex w-[calc(100%+48px)] min-w-0 flex-col gap-5 bg-white px-3 py-6 pb-12 text-gray-900 md:mx-auto md:w-full md:max-w-md md:px-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-lg font-semibold text-gray-900">発注書を確認・保存</h1>
          </div>
          <button
            type="button"
        ...
        {duplicate && (
          <div className="rounded-md border border-gray-200 bg-white p-3 text-sm">
            ...
            {!duplicate.canMutate && (
              <Link
                href={`${APP_ROUTES.TENANT.TOOL_DOCUMENTS}?type=purchase_order&open=${duplicate.id}`}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <label className="block space-y-1">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateLineItem(index, 'transaction_date', e.target.value || null)
                        }
                        disabled={readOnly || busy}
        ...
        >
          ...
          {flowStatus === 'saving' ? '保存中...' : duplicate?.canMutate ? '更新する' : '保存する'}
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="capture-form-shell -mx-[24px] flex w-[calc(100%+48px)] min-w-0 flex-col gap-5 bg-white px-3 py-6 pb-12 text-gray-900 md:mx-auto md:w-full md:max-w-md md:px-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-primary">文書ホルダー</p>
          <h1 className="text-lg font-semibold text-gray-900">発注書を撮る</h1>
        </div>
        <div className="flex shrink-0 items-center gap-3 text-sm">
          <Link
            href={`${APP_ROUTES.TENANT.TOOL_DOCUMENTS}?type=purchase_order`}
      ...
      {pages.length > 0 && (
        <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-900">撮影済み（{pages.length}ページ）</h2>
