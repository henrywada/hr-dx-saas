...
const MODE_META: Record<
  ReceiptMode,
  { label: string; headerKeys: readonly string[]; fieldLabels: Record<string, string> }
> = {
  expense: {
    label: "社内経費",
...
export function ReceiptAlbum({
  ...
  const [activeMode, setActiveMode] = useState<ReceiptMode>(initialMode);
  ...
  const detailMode: ReceiptMode =
    detail?.documentMode === "qualified_invoice" ? "qualified_invoice" : "expense";
  const detailMeta = MODE_META[detailMode];

  const syncDraft = useCallback((doc: DocumentDetail) => {
    const mode: ReceiptMode = doc.documentMode === "qualified_invoice" ? "qualified_invoice" : "expense";
    setHeaderDraft(normalizeHeader(doc.extracted, MODE_META[mode].headerKeys));
  ...
  const loadPage = useCallback(
    async (offset: number, append: boolean) => {
      ...
      try {
        ...
        console.error("receipt list load failed", err);
  ...
  const loadDetail = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        console.error("receipt detail load failed", err);
  ...
  function switchMode(mode: ReceiptMode) {
  ...
  async function handleReanalyze() {
    ...
    try {
      ...
      const nextHeader = normalizeHeader(result.extracted, detailMeta.headerKeys);
    ...
    <div className="mx-auto max-w-5xl px-6 py-8 pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-ink">領収書ホルダー</h1>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <Link
            href={`${APP_ROUTES.TENANT.TOOL_DOCUMENTS_NEW}?type=receipt`}
      ...
      <p className="mt-2 text-sm text-ink-soft">
        読み取った領収書を一覧表示します。タップで詳細・編集・再解析・削除ができます。
      </p>

      {/* Mode Tabs */}
      <div
        className="mt-4 inline-flex rounded-md border border-line bg-white p-0.5"
        role="tablist"
        aria-label="区分切替"
      >
          ...
          ([id, item]) => (
            ...
            >
              {id === "expense" ? (
                ...
                <FileCheck2 className="h-3.5 w-3.5" strokeWidth={1.75} aria-hidden />
              )}
              {item.label}
            </button>
          )
      ...
      <div className="mt-4 space-y-3 rounded-lg border border-line bg-white p-4">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium text-ink">取引日（開始）</span>
            <input
              type="date"
              value={fromDate}
          ...
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium text-ink">取引日（終了）</span>
            <input
              type="date"
              value={toDate}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5 truncate text-sm font-medium text-ink">
                    <span className="truncate">{item.title || "タイトル未設定"}</span>
                  ...
                  <p className="mt-0.5 truncate text-xs text-ink-soft">
                    {item.counterparty || "発行者未設定"}　{formatDate(item.contextDate)}
                  </p>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-medium text-signal">
                  領収書ホルダー・{detailMeta.label}
                </p>
                <h2 id="receipt-detail-title" className="text-base font-semibold text-ink">
                  {detail?.title || selectedSummary?.title || "領収書詳細"}
                </h2>
                <p className="mt-1 text-xs text-ink-soft">
                  {detail
                    ? `${detail.counterparty || "発行者未設定"} / ${formatDate(detail.contextDate)} / 更新 ${formatTimestamp(detail.updatedAt)}`
                    : "詳細を読み込んでいます"}
                </p>
              </div>
              <button
            ...
            {detail && (
              <div className="mt-4 space-y-4">
                ...
                {detail.images.length > 0 && (
                  <section>
                    <h3 className="mb-2 text-sm font-bold text-ink">画像</h3>
                    <p className="mb-2 text-xs text-ink-soft">タップすると拡大表示できます</p>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <section className="space-y-3">
                    <div className="rounded-lg border border-line bg-white p-4">
                      <h3 className="text-sm font-bold text-ink">項目</h3>
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        {detailMeta.headerKeys.map((key) => (
                          <label key={key} className="block space-y-1.5">
                            <span className="text-xs font-medium text-ink-soft">
                              {detailMeta.fieldLabels[key]}
                            </span>
                            <input
                              type="text"
