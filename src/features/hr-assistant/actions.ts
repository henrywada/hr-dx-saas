...
/**
 * 法令 RAG の類似度がこの値未満ならオンデマンド収集を検討する。
 *
 * 埋め込みモデルごとに妥当な値が変わる点に注意。
 * - OpenAI text-embedding-3-small 時代: 実測レンジ 0.24〜0.40 で、0.55 だと
 *   全クエリが下回り毎回オンデマンド収集が走っていた。
 * - Gemini gemini-embedding-001 移行後: コーパスで十分カバーできている質問は
 *   0.62〜0.70 に出る（本番実測）。0.55 のままだとほぼ発動しなくなるため引き上げた。
 *
 * 0.60 は「カバー済みの質問では発動せず、手薄な話題では発動する」ことを狙った値だが、
 * 根拠は実測1クエリ分のため暫定。オンデマンド収集の発動頻度を見て調整すること。
 */
const LAW_MISS_SIMILARITY_THRESHOLD = 0.6
