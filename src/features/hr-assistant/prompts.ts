...
export function buildSystemPrompt(
  ...
  const base =
    hasRagContext || hasLawContext
      ? `${contextNote}\n参照資料・法令情報の中に質問と関連する内容があれば、それを踏まえて回答してください。` +
        '具体的な数値・日付・結論がまだ確定していない場合（審議会で検討中等）は、推測で断定せず' +
        '「現時点では未確定である」旨を明記した上で、資料に記載されている検討状況（論点・対立点・今後の見通し等）を' +
        '出典とともに紹介してください。参照資料・法令情報に質問と関連する記載が一切ない場合のみ、' +
        '推測せず「登録された資料には記載がありません」と述べてください。'
