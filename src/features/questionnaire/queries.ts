...
/**
 * アンケート一覧取得（管理画面用）
 * developer は system/tenant 両方表示、それ以外は参照可能なもの全件
 */
export async function getQuestionnaires(
  ...
  const { data, error } = await db
    ...
    `)
    .or(`creator_type.eq.system,tenant_id.eq.${tenantId}`)
...
export async function getQuestionnaireDetail(
  ...
  const questionsWithDetails: QuestionWithDetails[] = (questions ?? []).map(
    (qq: { id: string; questionnaire_id: string; section_id: string | null; question_type: string; question_text: string; scale_labels: string[] | null; is_required: boolean; sort_order: number }) => ({
      ...qq,
...
export async function getAssignedQuestionnaires(
  ...
  return (data ?? []).map((row: {
    ...
    assigned_at: string;
    questionnaire: { id: string; title: string; description: string | null; creator_type: string } | null;
