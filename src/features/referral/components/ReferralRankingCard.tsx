...
export function ReferralRankingCard({ ranking }: ReferralRankingCardProps) {
  if (ranking.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-[#e2e6ec] shadow-sm p-5">
        <h3 className="text-sm font-semibold text-[#24292f] mb-3">推薦ランキング</h3>
        <p className="text-sm text-[#57606a] text-center py-4">まだランキングデータがありません</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e2e6ec] shadow-sm p-5">
      <h3 className="text-sm font-semibold text-[#24292f] mb-4">推薦ランキング</h3>
            ...
            <li key={item.employee_id} className="flex items-center gap-3">
              ...
              <div className="flex-shrink-0 w-7 text-center">
                {rank === 1 ? (
                  ...
                  <Medal className="h-5 w-5 text-[#57606a] mx-auto" aria-label="2位" />
                ) : rank === 3 ? (
                  <Medal className="h-5 w-5 text-amber-600 mx-auto" aria-label="3位" />
                ) : (
                  <span className="text-sm font-medium text-[#57606a]">{rank}</span>
                )}
              </div>

              {/* 社員名 */}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#24292f] truncate">{item.employee_name}</p>
              </div>

              {/* 推薦数・入社数バッジ */}
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs text-[#57606a]">{item.total_nominations}件</span>
                {item.hired_count > 0 && (
