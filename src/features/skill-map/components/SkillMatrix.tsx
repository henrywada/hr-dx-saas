  ...
  return (
    <div className="overflow-x-auto">
      <table className="border-collapse text-sm">
        <thead>
          <tr>
            <th className="sticky left-0 bg-white border border-gray-200 px-3 py-2 min-w-40 text-left">
              従業員
            </th>
            {categories.map(cat => (
              <th
            ...
            <th className="border border-gray-200 px-3 py-2 bg-gray-50 text-center min-w-28">
              充足率
            </th>
          </tr>
          <tr>
            ...
            {skills.map(skill => (
              <th
                key={skill.id}
                className="border border-gray-200 px-1 py-2 bg-gray-50 text-center"
              >
                <button
        ...
        <tbody>
          {sortedRows.map(row => (
            <tr key={row.employee_id} className="hover:bg-blue-50/30">
              <td className="sticky left-0 bg-white border border-gray-200 px-3 py-2 min-w-40">
                ...
                {row.division_name && (
                  <div className="text-xs text-gray-500">{row.division_name}</div>
                )}
              </td>
              {skills.map(skill => (
          ...
          <tr className="bg-gray-50 font-medium">
            <td className="sticky left-0 bg-gray-50 border border-gray-200 px-3 py-2 text-sm text-gray-500">
              習得率
            </td>
            {skills.map(skill => (
              <td key={skill.id} className="border border-gray-200 p-1 text-center">
