...
export function ConsultationForm({ history }: ConsultationFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (selectedTags.length === 0) { setError('カテゴリを1つ以上選択してください'); return }
    setSubmitting(true)
    setError(null)
    const result = await sendConsultation({ categoryTags: selectedTags, message: message.trim() || undefined })
    setSubmitting(false)
  ...
  return (
    <div className="max-w-lg mx-auto py-8 px-4 space-y-6">
      <div className="flex items-center gap-3 mb-2">
        <button onClick={() => router.back()} className="text-sm text-gray-500 hover:text-primary">← 戻る</button>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border p-5 space-y-4">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-2">悩みのカテゴリ（複数選択可）*</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">詳しく教えてください（任意）</label>
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4} placeholder="どんなことで困っているか、できるだけ具体的に書いてください..." className="w-full border rounded px-3 py-2 text-sm resize-none" />
        </div>

        <button type="submit" disabled={submitting || selectedTags.length === 0} className="w-full bg-accent-orange text-white text-sm font-semibold rounded py-2.5 hover:opacity-90 disabled:opacity-50 transition-opacity">
              ...
              <div key={c.id} className="border-l-2 border-gray-200 pl-3">
                <div className="flex flex-wrap gap-1 mb-1">
                  ...
                  <span className={`text-xs rounded px-2 py-0.5 ml-auto ${
                    ...
                    c.status === 'replied' ? 'bg-blue-100 text-blue-600' :
                    'bg-yellow-100 text-yellow-600'
                  }`}>
                    {c.status === 'resolved' ? '解決済み' : c.status === 'replied' ? '返答あり' : '未返答'}
                  </span>
                </div>
