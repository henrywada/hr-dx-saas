...
> = {
  A: {
    icon: Brain,
    gradient: 'from-[#FD7601] to-orange-600',
    bgLight: 'bg-[#f6f8fa]',
    accent: 'text-[#FD7601]',
  },
  B: {
    ...
    <div ref={containerRef} className="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8 min-h-screen">
      ...
      <div className="mb-8 text-center space-y-3 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <p className="text-gray-500 text-sm">
          {period.questionnaire_type}問形式 ・ 実施期間: {period.start_date} 〜 {period.end_date}
        </p>
      </div>
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md pb-4 pt-4 mb-6 border-b border-gray-100 shadow-sm">
        <div className="flex justify-between text-sm font-semibold text-gray-500 mb-2">
          <span>回答進捗</span>
          <span className="text-[#FD7601] font-bold">{progressPercent}%</span>
        </div>
        <div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-linear-to-r from-[#FD7601] to-[#FD7601] transition-all duration-700 ease-out"
        ...
        <div className="flex items-center gap-2 mt-3 overflow-x-auto pb-1">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${
                  isActive
                    ? `bg-linear-to-r ${config.gradient} text-white shadow-md`
                    : isDone
                      ? `${config.bgLight} ${config.accent}`
          ...
          <button
            onClick={() => setCurrentStep(domainGroups.length)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${
              currentStep === domainGroups.length
                ? 'bg-linear-to-r from-purple-500 to-violet-600 text-white shadow-md'
                : isAllAnswered
      ...
      {currentCategory ? (
        ...
        <ConfirmationView
          ...
          onGoToCategory={idx => setCurrentStep(idx)}
        />
      )}
      ...
      <div className="flex items-center justify-between mt-8 pb-8">
        ...
        {currentStep < totalSteps - 1 && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium bg-linear-to-r from-[#FD7601] to-orange-600 text-white hover:from-orange-700 hover:to-orange-700 shadow-md disabled:opacity-30 disabled:cursor-not-allowed"
          >
            次へ
...
// カテゴリごとの質問一覧
function CategoryQuestionsView({
  categoryGroup,
  answers,
  onSelect,
}: {
  ...
  answers: AnswerMap
  onSelect: (qId: string, score: number) => void
}) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">
              領域{categoryGroup.category}: {categoryGroup.categoryLabel}
            </h2>
            <p className="text-sm text-white/80">
              {answeredCount} / {categoryGroup.questions.length} 問回答済み
            </p>
          </div>
        </div>
      </div>
      <div className="divide-y divide-gray-50">
        {categoryGroup.questions.map(question => {
          ...
          return (
            <div
              key={question.id}
              className={`px-6 py-5 transition-colors ${selectedScore !== undefined ? 'bg-gray-50/50' : ''}`}
            >
                  ...
                  return (
                    <button
                      ...
                      className={`relative px-3 py-2.5 rounded-xl text-sm font-medium transition-all border-2 ${
                        isSelected
                          ? `border-current ${config.accent} ${config.bgLight} shadow-sm`
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {isSelected && (
                            <div
                              className={`w-2 h-2 rounded-full ${config.accent.replace('text-', 'bg-')}`}
                            />
                          )}
                        </div>
...
function ConfirmationView({
  ...
  isPending: boolean
  onSubmit: () => void
  onGoToCategory: (idx: number) => void
}) {
  return (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800">
                    領域{cg.category}: {cg.categoryLabel}
                  </p>
