...
export interface StressCheckPeriod {
  id: string
  tenant_id: string
  /** 拠点に紐づく場合のみ。NULL はテナント単位（旧データ）または division ベース新方式 */
...
/** period_divisions ジャンクション行（stress_check_period_divisions） */
export interface StressCheckPeriodDivision {
  id: string
  period_id: string
  division_id: string
  tenant_id: string
...
/** フロントエンド用：period + 紐づく division ID リスト */
export interface StressCheckPeriodWithDivisions extends StressCheckPeriod {
  divisionIds: string[] // 新方式では必須。旧 establishment ベース期間は [] で扱う
}
