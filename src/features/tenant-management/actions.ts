...
async function sendInviteEmailToManager(supabase: SupabaseAdmin, email: string, userId: string) {
  ...
  if (tokenError || !recoveryToken) {
    throw new Error(
      'リカバリートークン生成失敗: ' + (tokenError?.message || 'トークンが返されませんでした')
    )
  }
  ...
  await sendMail({
    ...
    subject: 'パスワード設定',
    html:
      '<p>hr-dxシステムへパスワードを設定してください。パスワード設定は<a href="' +
      actionLink +
      '">ここ</a>。</p>\n<p>このメールの有効期限は' +
      expiryFormatted +
      'までです。</p>',
...
export async function createTenant(formData: TenantFormData): Promise<TenantActionResult> {
  ...
  try {
    ...
    if (rpcError || !userId) {
      ...
      return {
        success: false,
        error: '責任者ユーザー作成失敗: ' + (rpcError?.message || 'ユーザーIDが返されませんでした'),
      }
    }
...
export async function resendInviteEmail(
  ...
  try {
    ...
    if (empError || !manager?.user_id) {
      return {
        success: false,
        error: '責任者情報が見つかりません: ' + (empError?.message || 'user_idなし'),
      }
    }
...
export async function deleteTenant(tenantId: string): Promise<TenantActionResult> {
  ...
  try {
    // 0. プラン別テンプレートテナントは削除禁止（サインアップ時のコピー元が消えるため）
    ...
    if (employees && employees.length > 0) {
      for (const emp of employees) {
        if (emp.user_id) {
          const { error: delErr } = await supabase.rpc('delete_auth_user', {
