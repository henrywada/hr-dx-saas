      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {isEdit ? 'テナントの基本情報を更新します' : 'テナント情報と責任者を登録します'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            >
              <option value="free">無料プラン（〜30名）</option>
              <option value="plan100">プラン100（〜100名）</option>
              <option value="plan300">プラン300（〜300名）</option>
              <option value="plan500">プラン500（〜500名）</option>
              <option value="plan1000">プラン1000（〜1000名）</option>
            </select>
            {errors.plan_type && <p className="mt-1 text-xs text-red-600">{errors.plan_type}</p>}
          </div>

          {/* 契約終了日（任意） */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">契約終了日</label>
            <input
              type="date"
          ...
          {isEdit &&
            editingTenant &&
            (editingTenant.manager_name || editingTenant.manager_email) && (
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-100">
                ...
                <p className="text-xs text-gray-500 font-mono mt-1">
                  {editingTenant.manager_email || '未設定'}
                </p>
              </div>
            )}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            >
              {loading ? (isEdit ? '更新中...' : '登録中...') : isEdit ? '更新する' : '登録する'}
            </button>
          </div>
        </form>
      </div>
