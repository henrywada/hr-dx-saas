      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">テナントの新規登録・変更・削除を行います</p>
          </div>
        </div>
                  ...
                  <tr key={tenant.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="text-sm text-gray-600 tabular-nums">
                        {tenant.max_employees != null ? `${tenant.max_employees} 名` : '無制限'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="text-sm font-semibold text-gray-800 tabular-nums">
                        {tenant.company_doctor_count > 0
                          ? `${tenant.company_doctor_count} 名`
                          : '—'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {tenant.created_at ? formatDateInJST(tenant.created_at) : '—'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {tenant.contract_end_at ? formatDateInJST(tenant.contract_end_at) : '—'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
