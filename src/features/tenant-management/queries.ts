...
export async function getAllTenants(): Promise<TenantWithManager[]> {
  ...
  try {
    ...
    const countMap = new Map<
      string,
      { registered_user_count: number; company_doctor_count: number }
    >()
    ...
    try {
      ...
      if (empError) {
        ...
        for (const row of empRows) {
          ...
          const ar = (row as { app_role?: { app_role?: string } | { app_role?: string }[] })
            .app_role
    ...
    return tenants.map(t => {
      ...
      return {
        ...
        contract_end_at:
          t.contract_end_at != null && t.contract_end_at !== '' ? String(t.contract_end_at) : null,
...
/**
 * SaaS管理者用: 全テナントの従業員一覧を取得 (RLSバイパス)
 * テナント名・アプリロール名・メールアドレスを結合して返す
 */
export async function getSaasEmployees(): Promise<SaasEmployee[]> {
  const supabase = createAdminClient()
  ...
  try {
    ...
    if (error) {
      console.error('getSaasEmployees error:', error)
      return []
    }
    ...
    return (employees ?? [])
      .map(e => {
        const emp = e as {
          ...
          user_id: string | null
          app_role:
            | { app_role?: string; name?: string }
            | { app_role?: string; name?: string }[]
            | null
        }
        const role = Array.isArray(emp.app_role) ? emp.app_role[0] : emp.app_role
        const tenantId = String(emp.tenant_id ?? '')
        return {
          id: String(emp.id),
          tenant_id: tenantId,
          tenant_name: tenantNameMap.get(tenantId) ?? '—',
          employee_no: emp.employee_no ?? null,
          name: emp.name ?? '—',
          email: emp.user_id ? (emailMap.get(emp.user_id) ?? null) : null,
          sex: emp.sex ?? null,
          app_role_name: role?.name ?? null,
        }
      })
      .sort((a, b) => {
        // 従業員番号 → 氏名 の順でソート
        const noA = a.employee_no ?? '￿'
        const noB = b.employee_no ?? '￿'
        if (noA !== noB) return noA.localeCompare(noB, 'ja')
        return a.name.localeCompare(b.name, 'ja')
      })
  } catch (err) {
    console.error('getSaasEmployees 予期せぬエラー:', err)
    return []
  }
}
