...
export function RiskRankingTable({ rows }: Props) {
  const [detailRow, setDetailRow] = useState<TurnoverRiskRow | null>(null)
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-200">
                <th className="border-b border-gray-300 px-4 py-1 text-left text-sm font-semibold text-gray-800">
                  順位
                </th>
                <th className="border-b border-gray-300 px-4 py-1 text-left text-sm font-semibold text-gray-800">
                  氏名
                ...
                <th className="border-b border-gray-300 px-4 py-1 text-center text-sm font-semibold text-gray-800">
                  リスクレベル
                </th>
                <th className="border-b border-gray-300 px-4 py-1 text-center text-sm font-semibold text-gray-800">
                  スコア
            ...
            <tbody>
              {rows.map((row, idx) => (
                ...
                >
                  <td className="border-b border-gray-200 px-4 py-1 text-gray-500">{idx + 1}</td>
                  <td className="border-b border-gray-200 px-4 py-1 font-medium text-gray-900">
                    {row.employee_name}
                  </td>
                  <td className="border-b border-gray-200 px-4 py-1 text-gray-600">
                    {row.department_name ?? '—'}
                  </td>
                  <td className="border-b border-gray-200 px-4 py-1 text-center">
      ...
      {detailRow && <RiskDetailModal row={detailRow} onClose={() => setDetailRow(null)} />}
