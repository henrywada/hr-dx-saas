...
export function Header({ role = "member", userName = "Guest User", onMenuClick }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (user) {
          // 優先順: employeesテーブルの name > user_metadata.name > email
          let name = user.user_metadata?.name ?? user.email ?? localName;
          let roleFromMeta = user.user_metadata?.role ?? localRole;

          // employees テーブルに対応するレコードがあれば name を上書き（より正確な表示）
          try {
            ...
            if (!empErr && employee?.name) {
              name = employee.name;
              // app_role からロール推定が必要なら設定する（optional）
              const distinctRole = (employee as any).app_role;
              if (distinctRole?.app_role) {
                // 例: 'admin', 'employee', 'supaUser' など
                roleFromMeta = distinctRole.app_role;
              }
            }
          } catch (e) {
            // employeesが参照できない場合は無視（ログやモニタに残すとよい）
          }

          setLocalName(name);
          setLocalRole(roleFromMeta);
        }
      } catch (e) {
        // Supabase の取得に失敗した場合は何もしない（既存の表示を維持）
  ...
  const isEmployee = localRole === "employee";
  const label = isEmployee ? "従業員" : (localRole === "admin" ? "管理者" : (localRole === "supaUser" ? "SaaS管理者" : "メンバー"));
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="flex items-center gap-3 pl-2 cursor-pointer hover:opacity-80 transition-opacity">
          <div className="text-right hidden md:block leading-tight">
            <div className="text-sm font-semibold text-white">{localName}</div>
            <div className={`text-[10px] px-2 py-0.5 rounded-full inline-block mt-1 font-medium tracking-wide shadow-sm border ${
              localRole === 'supaUser' ? 'bg-gradient-to-r from-purple-50 to-white text-purple-700 border-purple-200' :
              localRole === 'admin' ? 'bg-gradient-to-r from-orange-50 to-white text-orange-700 border-orange-200' :
          ...
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-600 font-bold border-2 border-white/50 shadow-md ring-1 ring-white/20">
            {typeof localName === "string" && localName.length > 0 ? localName.slice(0, 1) : "G"}
          </div>
        </div>
      </div>
    </header>
