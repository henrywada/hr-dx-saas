-- このファイルはクラウド本番の既存データを追加投入するもの。
-- ローカルの既存データ（重複含む）は変更しない。今後このテーブル群を
-- 更新する際は別途新規マイグレーションで追加すること。
-- クラウド→ローカル復元（service_class / service_category / service_class_index / service）
-- INSERT ... ON CONFLICT (id) DO NOTHING（追加のみ）
